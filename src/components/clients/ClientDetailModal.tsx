...
import { Client, Sale, Flight } from "../../types";
...
export function ClientDetailModal({
  ...
  const clientFlights = flights.filter((f) => f.passenger === client.name);
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 gap-4 bg-gray-50 p-4 rounded-lg">
        <div>
          <span className="text-gray-500 text-sm block">Tipo Doc:</span>{" "}
          <span className="font-medium">{client.docType}</span>
        </div>
        <div>
          <span className="text-gray-500 text-sm block">Numero:</span>{" "}
          <span className="font-medium">{client.docNumber}</span>
        </div>
        <div>
          <span className="text-gray-500 text-sm block">Telefono:</span>{" "}
          <span className="font-medium">{client.phone}</span>
        </div>
        <div>
          <span className="text-gray-500 text-sm block">Correo:</span>{" "}
          <span className="font-medium">{client.email}</span>
        </div>
        <div>
          <span className="text-gray-500 text-sm block">F. Nacimiento:</span>{" "}
          <span className="font-medium">
            {client.birthDate ? formatDate(client.birthDate) : "N/A"}
          </span>
        </div>
        <div>
          <span className="text-gray-500 text-sm block">Registro:</span>{" "}
          <span className="font-medium">{formatDate(client.registrationDate)}</span>
        </div>
      </div>
              ...
              >
                <div className="flex items-center gap-2">
                  <Plane
                    size={12}
                    className={
                      flight.type === "ida" ? "text-blue-500" : "text-indigo-500 rotate-180"
                    }
                  />

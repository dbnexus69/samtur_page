...
export function PermissionsGrid({ permissions, onChange }: PermissionsGridProps) {
  const toggle = (module: keyof RolePermissions, type: string) => {
    ...
    const modulePerms = { ...next[module] } as Record<string, unknown>;
    if (type in modulePerms) {
      ...
      (next as Record<string, unknown>)[module] = modulePerms;
      onChange(next as RolePermissions);
    }
  };
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
      {modules.map((mod) => (
        <div
          key={mod.id}
          className="p-5 border border-gray-border rounded-2xl bg-white shadow-sm hover:shadow-md transition-shadow"
        >
          ...
          <div className="flex flex-col gap-3">
            {Object.keys(permissions[mod.id]).map((permKey) => {
              const val = (permissions[mod.id] as Record<string, unknown>)[permKey];
              const displayLabel = permLabels[permKey] || permKey;

              return (
                <div
                  key={permKey}
                  className="flex items-center justify-between p-2.5 bg-gray-50/50 hover:bg-gray-50 rounded-xl transition-colors border border-transparent hover:border-gray-100"
                >
                  ...
                  {typeof val === "boolean" ? (
                    ...
                    >
                      <div
                        className={`bg-white w-3.5 h-3.5 rounded-full shadow-sm transform transition-transform duration-300 ${
                    ...
                    <select
                      ...
                      onChange={(e) => {
                        const next = { ...permissions };
                        (next[mod.id] as Record<string, unknown>)[permKey] = e.target.value;
                        onChange(next as RolePermissions);
                      }}
                    >
